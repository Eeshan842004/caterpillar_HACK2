...
def _finding(machine_id, zone_id, reason, minutes, local_date="2026-09-23"):
    return {"finding_id": str(uuid.uuid4()), "subject_id": str(uuid.uuid4()), "pattern_code": "idle_reported_wait",
            "owner": "site", "evidence_status": "reported", "reason_key": "finding.idle_reported_wait",
            "observed": {"text_key": "finding.observed.idle_wait", "params": {}}, "possible_explanations": [reason],
            "related_entry_ids": [], "machine_id": machine_id, "site_id": "SITE-CHN-01", "zone_id": zone_id,
            "reason_code": reason, "minutes": minutes, "local_date": local_date, "rule_version": "findings@1"}
...
def test_site_delay_follow_up_groups_and_moves_on_correction(client, db):
    ...
    findings = [entry(device_id, "EX-07", "inference", "finding", "inferred",
                      _finding("EX-07", "Z-CHN-LB2", "waiting_truck", m), rule_or_model_version="findings@1")
                for m in (15, 20, 20)]
    push(client, device_id, secret, findings)
    fu = db.query(FollowUp).filter(FollowUp.group_key == "site_delay:SITE-CHN-01:Z-CHN-LB2:waiting_truck:2026-09-23").one()
    assert fu.metrics == {"count": 3, "total_minutes": 55.0}
    assert fu.title == "Truck wait at Loading Bay 2 — 3 reports, 55 min total today"

    replacement = dict(findings[0]["payload"], reason_code="access_blocked", possible_explanations=["access_blocked"])
    correction = entry(device_id, "EX-07", "correction", "finding", "inferred",
                       {"target_kind": "inference", "target_subtype": "finding", "replacement": replacement},
                       supersedes=findings[0]["entry_id"], rule_or_model_version="findings@1")
    push(client, device_id, secret, [correction])
    db.expire_all()
    assert db.get(FollowUp, fu.follow_up_id).metrics == {"count": 2, "total_minutes": 40.0}
    moved = db.query(FollowUp).filter(FollowUp.group_key == "site_delay:SITE-CHN-01:Z-CHN-LB2:access_blocked:2026-09-23").one()
...
def test_offline_completion_of_reassigned_task_is_a_conflict(client, db):
    ...
    task = db.get(TaskAssignment, "T-20260923-EX07-2") or db.query(TaskAssignment).filter(
        TaskAssignment.machine_id == "EX-07", TaskAssignment.sequence == 2).one()
    task.machine_id, task.revision = "EX-01", 2
    db.commit()
    device_id, secret = pair(client)
    done = entry(device_id, "EX-07", "task_event", "complete", "reported",
...
def _chain(entries):
    ...
    for i, e in enumerate(entries, start=1):
        ...
        body = {k: e[k] for k in ("entry_id", "device_id", "shift_id", "machine_id", "operator_id", "kind", "subtype",
                                  "source", "payload")}
        body.update(observed_at=ms(e["observed_at"]), recorded_at=ms(e["recorded_at"]), supersedes=None,
                    original_text=None)
        canonical = json.dumps(body, sort_keys=True, separators=(",", ":"), ensure_ascii=False)
        e.update(chain_seq=i, prev_hash=prev, canonical_payload=canonical,
...
def _incident_entries(device_id, incident_id):
    ...
    created = entry(device_id, "EX-07", "incident", "created", "observed", {
        "incident_id": incident_id, "origin": "auto", "trigger_alert_id": "a1", "occurred_at": now,
        "zone_id": "Z-CHN-TR1", "observed": {"machine_state": "WORKING", "speed_kmh": 1.0,
                                             "detected": [{"object_id": "P1", "type": "person", "place": "rear",
                                                           "min_distance_m": 3.1}]},
        "snapshot": {"pre_s": 60, "post_s": 30, "samples": [], "proximity": [],
                     "alerts": [{"ts": 0, "alert_type": "A-PROX-WARN", "level": "WARNING"}]},
        "snapshot_complete": False, "severity_default": "high"}, audience="safety")
    report = entry(device_id, "EX-07", "report", "incident_report", "reported", {
        "incident_id": incident_id, "type": "near_miss", "object": "person", "place": "rear", "contact": "no",
        "severity": "high", "via": "voice"}, audience="safety")
...
def test_incident_chain_verifies_and_tampering_is_detected(client, db):
    ...
    res = client.post("/api/v1/devices/pair", json={"pairing_code": "100007", "machine_id": "EX-07",
                                                     "device_label": "t2", "client_device_id": other_id})
...
def test_incident_review_is_site_scoped_and_writes_ledger(client, db):
    ...
    res = client.post(f"/api/v1/console/incidents/{incident_id}/review",
    ...
    reviewed = db.query(LedgerEntry).filter(LedgerEntry.kind == "incident", LedgerEntry.subtype == "reviewed").all()
