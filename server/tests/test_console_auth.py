...
def test_follow_up_resolve_requires_header_and_role(client, db):
    ...
    fu = FollowUp(
        ...
        group_key="machine_check:EX-07:test",
        title="Belt switch flapping on EX-07",
        summary="test",
        priority=2,
        metrics={},
    ...
    assert (
        client.post(
            f"/api/v1/console/follow-ups/{fu.follow_up_id}/resolve", json={"note": "switch replaced"}
        ).status_code
        == 403
    )  # missing X-Requested-With
    res = client.post(
        f"/api/v1/console/follow-ups/{fu.follow_up_id}/resolve",
        json={"note": "switch replaced"},
        headers=MUTATE,
    )
    ...
    assert (
        client.get(f"/api/v1/console/follow-ups/{fu.follow_up_id}").status_code == 404
    )  # not a trainer category
