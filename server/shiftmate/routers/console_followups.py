...
from shiftmate.services.sos import sos_wire
...
def _summary(fu: FollowUp) -> FollowUpSummary:
    return FollowUpSummary(
        ...
        zone_id=fu.zone_id,
        reason_code=fu.reason_code,
        metrics=fu.metrics or {},
        first_seen_at=iso_ms(fu.first_seen_at),
        last_seen_at=iso_ms(fu.last_seen_at),
    )
...
def _load(db: Session, user: ConsoleUser, follow_up_id: str, *, write: bool) -> FollowUp:
    ...
    if (
        fu is None
        or fu.site_id not in (user.site_ids or [])
        or fu.category not in ROLE_CATEGORIES.get(user.role, [])
    ):
    ...
    if write and (user.role, fu.category) in READ_ONLY:
        raise ShiftMateException(
            status_code=403, code="forbidden", message="Your role can read but not act on this item."
        )
    return fu
...
def _detail(db: Session, fu: FollowUp) -> FollowUpDetail:
    contributions = []
    for c in (
        db.query(FollowUpContribution).filter(FollowUpContribution.follow_up_id == fu.follow_up_id).all()
    ):
        ...
        contributions.append(
            Contribution(
                ...
                minutes=c.minutes,
                active=c.active,
                machine_id=root.machine_id if root else None,
                operator_display=op.display_name if op else None,
                observed_at=iso_ms(root.observed_at) if root else None,
                original_text=reason_entry.original_text if reason_entry else None,
            )
        )
    comments = [
        Comment(comment_id=c.comment_id, user_id=c.user_id, text=c.text, created_at=iso_ms(c.created_at))
        for c in db.query(FollowUpComment)
        .filter(FollowUpComment.follow_up_id == fu.follow_up_id)
        .order_by(FollowUpComment.created_at)
        .all()
    ]
    related = {}
    if fu.category == "sos" and fu.related_id:
        ev = db.get(SosEvent, fu.related_id)
        related = {"related_id": fu.related_id, "sos": sos_wire(db, ev) if ev else None}
    elif fu.related_id:
        entry = db.get(LedgerEntry, fu.related_id)
        related = {
            "related_id": fu.related_id,
            "entry": {"kind": entry.kind, "subtype": entry.subtype, "payload": entry.payload}
            if entry
            else None,
        }
    return FollowUpDetail(
        **_summary(fu).model_dump(),
...
def list_followups(
    ...
    if cursor:
        ...
        q = q.filter(
            or_(
                FollowUp.priority > prio,
                and_(FollowUp.priority == prio, FollowUp.first_seen_at > first_dt),
                and_(
                    FollowUp.priority == prio, FollowUp.first_seen_at == first_dt, FollowUp.follow_up_id > fid
                ),
            )
        )
    rows = (
        q.order_by(FollowUp.priority.asc(), FollowUp.first_seen_at.asc(), FollowUp.follow_up_id.asc())
        .limit(limit + 1)
        .all()
    )
    page = rows[:limit]
    nxt = (
        encode_cursor([page[-1].priority, page[-1].first_seen_at.isoformat(), page[-1].follow_up_id])
        if len(rows) > limit
        else None
    )
...
def get_followup(
    follow_up_id: str, db: Session = Depends(get_db), user: ConsoleUser = Depends(get_current_user)
):
...
def assign_followup(
    ...
    if fu.status == "resolved":
        raise ShiftMateException(
            status_code=409, code="invalid_state", message="Follow-up is already resolved."
        )
    fu.assigned_to = req.user_id
    fu.status = "assigned" if req.user_id else "open"
    fu.updated_at = utc_now()
    notify(db, fu.site_id, ["follow-ups"])
    db.commit()
...
def resolve_followup(
    ...
    now = utc_now()
    fu.status, fu.resolved_at, fu.resolved_by, fu.resolution_note, fu.updated_at = (
        "resolved",
        now,
        user.user_id,
        req.note,
    ...
    for machine_id in sorted(machines):
        record_change(
            ...
            machine_id,
            {
                "follow_up_id": fu.follow_up_id,
                "category": fu.category,
                "related_entry_ids": [c.entry_id for c in contributions],
    ...
    if fu.category == "alert_review" and fu.related_id:
        ...
        raised = (
            db.query(LedgerEntry).filter(LedgerEntry.kind == "alert", LedgerEntry.subtype == "raised").all()
        )
        ...
        if source is not None:
            server_entry(
                ...
                audience="site",
                payload={
                    "alert_id": fu.related_id,
                    "alert_type": source.payload["alert_type"],
                    "follow_up_id": fu.follow_up_id,
                    "note": req.note or None,
                },
            )
...
def comment_followup(
    ...
    c = FollowUpComment(
        follow_up_id=fu.follow_up_id, user_id=user.user_id, text=req.text, created_at=utc_now()
    )
    db.add(c)
    notify(db, fu.site_id, ["follow-ups"])
    db.commit()
