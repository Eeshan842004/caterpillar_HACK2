...
def reset_rate_limits():
    for limiter in (ratelimit.login_per_ip, ratelimit.login_per_user, ratelimit.pairing_per_ip):
        limiter.reset()
    yield
...
def make_device_auth_headers(method, path, body_bytes, device_id, device_secret=None, timestamp_ms=None) -> dict:
...
def pair(client, code="100007", machine="EX-07") -> tuple[str, str]:
    res = client.post("/api/v1/devices/pair", json={"pairing_code": code, "machine_id": machine, "device_label": "test"})
...
def entry(device_id, machine_id, kind, subtype, source, payload, audience="site", **extra) -> dict:
    ...
    base = {"entry_id": str(uuid.uuid4()), "device_id": device_id, "shift_id": None, "machine_id": machine_id,
            "operator_id": "OP-0007", "kind": kind, "subtype": subtype, "source": source, "payload": payload,
            "observed_at": ts, "recorded_at": ts, "audience": audience, "data_origin": "live"}
    base.update(extra)
    return base


def push(client, device_id, device_secret, entries: list[dict]) -> dict:
    batch = {"batch": [{"entry_id": e["entry_id"], "type": "report", "created_at": e["recorded_at"],
                        "body": {"kind": "ledger_entry", "entry": e}} for e in entries]}
