...
EXPECTED_COLUMNS = {
    "sites.csv": [
        "site_id", "name", "sector", "lat", "lon", "utc_offset_minutes",
        "diesel_price_inr_per_l", "dark_start_local", "dark_end_local",
        "job_efficiency_override", "congestion_level", "data_origin", "created_at"
    ],
    "zones.csv": [
        "zone_id", "site_id", "name", "kind", "center_lat", "center_lon",
        "radius_m", "speed_limit_kmh", "data_origin", "created_at"
    ],
    "operators.csv": [
        "operator_id", "site_id", "display_name", "language", "skill_level",
        "experience_months", "hired_at", "regular_machine_class", "data_origin", "created_at"
    ],
    "machines.csv": [
        "machine_id", "short_id", "site_id", "profile_id", "profile_version",
        "machine_class", "model_name", "year_of_manufacture", "detail_level",
        "data_origin", "created_at"
    ],
    "fleet.csv": [
        "machine_id", "short_id", "site_id", "machine_class", "profile_id",
        "profile_version", "state", "open_alerts", "status_observed_at",
        "last_sync_at", "detail_level", "data_origin"
    ],
    "shifts.csv": [
        "shift_id", "site_id", "machine_id", "operator_id", "started_at",
        "ended_at", "local_shift_date", "shift_code", "data_origin", "recorded_at"
    ],
    "tasks.csv": [
        "task_id", "assignment_revision", "site_id", "zone_id", "machine_id",
        "operator_id", "shift_id", "task_type", "material", "quantity", "unit",
        "location_text", "priority", "completion_criterion", "assignment_source",
        "planned_date", "sequence", "planned_start_at", "planned_start_window_min",
        "estimate_requested_at", "planner_minutes", "profile_id", "profile_version",
        "skill_level_at_start", "experience_months_at_start", "machine_age_years_at_start",
        "condition_id", "weather_at_start", "visibility_at_start", "temperature_band_at_start",
        "time_of_day_at_start", "site_congestion_at_start", "darkness_at_start",
        "baseline_minutes", "status", "actual_start_at", "actual_end_at",
        "actual_active_min", "actual_waiting_min", "actual_break_min",
        "actual_paused_min", "actual_elapsed_min", "output_qty", "completion_quality",
        "training_eligible", "training_exclusion_reason", "split_temporal",
        "split_unseen_operator", "data_origin", "recorded_at"
    ],
    "conditions_hourly.csv": [
        "condition_id", "site_id", "valid_from", "valid_to", "issued_at",
        "weather", "visibility", "visibility_m", "temp_c", "heat_index_c",
        "wind_kmh", "precipitation_mm", "source", "data_origin", "recorded_at"
    ],
    "summaries_5m.csv": [
        "summary_id", "site_id", "machine_id", "shift_id", "task_id",
        "window_start", "window_end", "recorded_at", "engine_on_s", "secured_s",
        "ready_s", "working_s", "travelling_s", "unknown_s", "idle_s",
        "fuel_used_l", "load_cycles", "max_speed_kmh", "avg_load_factor_pct",
        "belt_unfastened_moving_s", "samples", "missing_samples",
        "missing_signal_names", "profile_id", "profile_version", "data_origin"
    ],
    "idle_events.csv": [
        "idle_event_id", "site_id", "machine_id", "operator_id", "shift_id",
        "task_id", "zone_id", "started_at", "ended_at", "recorded_at",
        "duration_s", "required_s", "non_required_s", "required_basis",
        "idle_class", "reason_code", "reason_entry_id", "reason_recorded_at",
        "category", "prompted_at", "prompt_answered_at", "sensor_coverage_pct",
        "evidence_status", "data_origin"
    ],
    "alerts.csv": [
        "alert_event_id", "alert_id", "site_id", "machine_id", "shift_id",
        "task_id", "zone_id", "transition", "alert_type", "level", "group_key",
        "occurrences", "object_id", "object_type", "place", "distance_m",
        "ttc_s", "multiplier", "clear_reason", "exit_cue", "safe_exit_json",
        "observed_at", "recorded_at", "source", "rule_or_model_version",
        "profile_id", "profile_version", "data_origin"
    ],
}
...
def validate_generated_bundle(bundle_dir: Path = GENERATED_DIR) -> Tuple[bool, str]:
    ...
    if jsonl_path.exists():
        ...
        if man_jsonl:
            ...
            check(man_jsonl["rows"] == len(ledger_rows), f"ledger_entries.jsonl count ({len(ledger_rows)}) matches manifest", 1)
    ...
    shifts = parsed_csvs.get("shifts.csv", [])
    ...
    check(len(operator_ids) == 48 and len(operator_ids) == len(operators), "operators.csv has exactly 48 unique operators", 2)
    check(len(machine_ids) == 24 and len(machine_ids) == len(machines), "machines.csv has exactly 24 detailed machines", 2)
    check(len(fleet_ids) == 100 and len(fleet_ids) == len(fleet), "fleet.csv has exactly 100 machines (24 detailed + 76 status_only)", 2)
    ...
    check(len(bad_task_site_fks) == 0 and len(bad_task_op_fks) == 0 and len(bad_task_m_fks) == 0, "Task foreign keys resolve cleanly", 2)
    ...
    for t in tasks:
        ...
        if t["actual_start_at"] and t["actual_end_at"]:
            if t["actual_start_at"] >= t["actual_end_at"]:
                task_time_errors += 1
    ...
    for i in idles:
        if i["reason_recorded_at"] and i["started_at"]:
            if i["reason_recorded_at"] < i["started_at"]:
    ...
    for aid, transitions in alert_grouped.items():
        ...
        if raised and cleared:
            if cleared["observed_at"] < raised["observed_at"]:
    ...
    for t in tasks:
        if t["status"] == "COMPLETED" and t["actual_active_min"]:
            ...
            dt_start = datetime.fromisoformat(t["actual_start_at"].replace("Z", "+00:00"))
            dt_end = datetime.fromisoformat(t["actual_end_at"].replace("Z", "+00:00"))
            clock_min = (dt_end - dt_start).total_seconds() / 60.0
            if abs(clock_min - elapsed) > 0.15:
                math_errors += 1

            # Training eligible active target > 0
            if t["training_eligible"] == "true":
                if active <= 0:
            ...
            if prof:
                ...
                job_override = float(s_obj["job_efficiency_override"]) if s_obj.get("job_efficiency_override") else None
                ...
                if recomputed_b is not None:
                    if abs(recomputed_b - float(t["baseline_minutes"])) > 0.15:
    ...
    check(baseline_recomp_errors == 0, "Task baseline_minutes recomputes exactly from profile and site job efficiency", 4)

    # --- GATE 5: Sensor Dropouts and Missing Signals ---
    dropout_rows = [s for s in summaries if s["missing_signal_names"] != "[]"]
    signal_column = {"fuel_used_l": "fuel_used_l", "ground_speed_kmh": "max_speed_kmh",
                     "load_factor_pct": "avg_load_factor_pct", "seatbelt_fastened": "belt_unfastened_moving_s"}
    valid_dropouts = [
        s for s in dropout_rows
        if all(s[signal_column[name]] == "" for name in json.loads(s["missing_signal_names"]) if name in signal_column)
    ]
    check(len(dropout_rows) > 0, f"Simulated sensor dropouts present ({len(dropout_rows)} summaries)", 5)
    check(len(valid_dropouts) == len(dropout_rows), "Sensor dropouts produce null metric values, never 0 or false", 5)

    idle_classes = {i["idle_class"] for i in idles}
    check("reported" in idle_classes and "unexplained" in idle_classes, "Idle events contain both reported and unexplained classes", 5)
    ...
    for t in tasks:
        ...
        if est:
            ...
            pairs = [("weather", "weather_at_start"), ("visibility", "visibility_at_start"),
                     ("temperature_band", "temperature_band_at_start"), ("time_of_day", "time_of_day_at_start"),
                     ("site_congestion", "site_congestion_at_start"), ("darkness", "darkness_at_start")]
            if any(cell(ctx.get(k)) != t[col] for k, col in pairs):
                mismatched_context_count += 1

    check(mismatched_context_count == 0, "inference/estimate.context equals task *_at_start pre-start columns", 6)
    ...
    check(len(split_a_train) > 0 and len(split_a_calib) > 0 and len(split_a_test) > 0, "Split A (temporal) covers train, calibration, test", 7)

    held_out_cfg = manifest.get("split_policy", {}).get("unseen_operator", {})
    held_out_ops = set(held_out_cfg.get("held_out_excavators", []) + held_out_cfg.get("held_out_haul_trucks", []))
    held_out_tasks = [t for t in tasks if t["operator_id"] in held_out_ops and t["training_eligible"] == "true"]
    bad_split_b = [t for t in held_out_tasks if t["split_unseen_operator"] != "test"]
    check(len(held_out_tasks) > 0 and len(bad_split_b) == 0, "Split B (unseen operators) assigns all held-out tasks to test", 7)
    ...
    for entries in chains.values():
        ...
        for i, e in enumerate(entries, start=1):
            ...
            rec = datetime.fromisoformat(e["recorded_at"].replace("Z", "+00:00"))
            body = {k: e[k] for k in ("entry_id", "device_id", "shift_id", "machine_id", "operator_id",
                                      "kind", "subtype", "source", "payload", "supersedes", "original_text")}
            ...
            if e["chain_seq"] != i or e["prev_hash"] != prev or e["canonical_payload"] != canon or e["content_hash"] != expect:
                chain_errors += 1
            prev = e["content_hash"]
    n_chained = sum(len(v) for v in chains.values())
    check(chain_errors == 0 and n_chained > 0, f"Incident hash chains verify per §8.10 ({n_chained} chained entries)", 1)
    ...
    for cls in sorted({m_class[t["machine_id"]] for t in done}):
        ...
        check(levels == {"beginner", "intermediate", "expert"}, f"{cls} tasks include all three skill levels", 2)
    ...
    check(len(joined) >= 0.99 * len(tasks), f"{len(joined)}/{len(tasks)} tasks joined to their pre-start forecast hour", 3)
    wx_mismatch = sum(1 for t in joined if derive_weather(cond_by_id[t["condition_id"]]) != t["weather_at_start"])
    ...
    check(any(i["idle_class"] == "required" for i in idles), "Required idle (warm-up / cool-down) is present", 5)
    ...
    check(bool(speed_drop) and all(s["max_speed_kmh"] == "" for s in speed_drop), "Speed dropouts null max_speed_kmh", 5)
    ...
    required_kinds = [
        ("shift_event", "start"), ("shift_event", "end"), ("task_event", "start"), ("task_event", "pause"),
        ("task_event", "resume"), ("task_event", "block"), ("task_event", "complete"), ("task_event", "cancel"),
        ("inference", "estimate"), ("observation", "signal_summary_5m"), ("observation", "condition_forecast"),
        ("idle_event", "started"), ("idle_event", "ended"), ("inference", "idle_classification"),
        ("report", "idle_reason"), ("correction", "idle_reason"), ("alert", "raised"), ("alert", "acknowledged"),
        ("alert", "cleared"), ("incident", "created"), ("incident", "snapshot_completed"),
        ("report", "incident_report"), ("inference", "incident_extraction"), ("inference", "finding"),
        ("inference", "recommendation"), ("learning_event", "completed"), ("handover_item", "added"),
        ("handover_item", "acknowledged"), ("report", "handover_note"),
    ]
    missing_kinds = [f"{k}/{st}" for k, st in required_kinds if (k, st) not in kinds]
    check(not missing_kinds, f"All required ledger kinds are generated {missing_kinds if missing_kinds else ''}", 6)
    valid_types = {"A-BELT-MOVE", "A-BELT-OPER", "A-BELT-UNAV", "A-PROX-CAUT", "A-PROX-WARN", "A-PROX-CRIT",
                   "A-PROX-UNAV", "A-SPEED", "A-HEAT", "A-WIND", "A-IDLE-ASK", "A-EXIT-UNSEC", "A-SOS"}
    bad_types = {a["alert_type"] for a in alerts} - valid_types
    check(not bad_types, f"Alert types are all in the §5.1 AlertType enum {sorted(bad_types) if bad_types else ''}", 6)
    corrections = {r["supersedes"]: r["payload"]["replacement"]["reason_code"] for r in ledger_rows
                   if r["kind"] == "correction" and r["subtype"] == "idle_reason"}
    bad_effective = [i for i in idles if i["reason_entry_id"] in corrections
                     and i["reason_code"] != corrections[i["reason_entry_id"]]]
    check(bool(corrections) and not bad_effective,
          f"idle_events shows the corrected (effective) reason ({len(corrections)} corrections)", 6)
    bad_cancel = [t for t in tasks if t["status"] == "CANCELLED"
                  and (t["actual_active_min"] or t["training_eligible"] == "true")]
    ...
    check(sum(t["task_type"] == "trenching" for t in ravi) == 1, "OP-0007 (Ravi) has exactly 1 completed trenching task", 2)
    check(all(t["weather_at_start"] not in ("rain", "dusty") and t["darkness_at_start"] == "false" for t in ravi),
          "OP-0007 never started a task in rain, dust or darkness (condition-prep demo)", 2)
