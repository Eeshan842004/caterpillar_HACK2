...
from shiftmate.services.tasks import reassign_task, task_wire
...
def _resolve_group(db: Session, group_key: str, user: ConsoleUser, note: str | None) -> None:
    ...
    if fu:
        fu.status, fu.resolved_at, fu.resolved_by, fu.resolution_note = (
            "resolved",
            utc_now(),
            user.user_id,
            note,
...
def decide_reassignment(
    ...
    if req.decision == "accept":
        ...
        rr.status = "rejected"
    rr.decided_by, rr.decided_at, rr.new_machine_id, rr.decision_note = (
        user.user_id,
        utc_now(),
        req.new_machine_id,
        req.note,
    ...
    record_change(
        ...
        old_machine,
        {
            ...
            "new_machine_id": req.new_machine_id,
            "note": req.note,
        },
    )
    _resolve_group(db, f"reassign:{request_id}", user, req.note)
    notify(db, task.site_id, ["follow-ups", "tasks"])
    db.commit()
    return {"request": {"request_id": rr.request_id, "status": rr.status}, "task": task_wire(task)}
...
def list_handovers(
    ...
    site_machines = {
        m.machine_id for m in db.query(Machine).filter(Machine.site_id.in_(user.site_ids or [])).all()
    }
        ...
        out.append(
            {
                "handover": {
                    ...
                    "voice_note_upload_id": ho.voice_note_upload_id,
                    "wording_method": ho.wording_method,
                },
                "items": [
                    {
                        "item_id": i.item_id,
...
def resolve_handover_item(
    item_id: str,
    req: HandoverItemResolveRequest,
    db: Session = Depends(get_db),
    ...
    item.status, item.resolved_by, item.resolved_at, item.resolution_note = (
        "resolved",
        user.user_id,
        utc_now(),
        req.note,
    ...
    notify(db, machine.site_id, ["handovers"])
    db.commit()
    return {
        "item_id": item.item_id,
        "status": item.status,
...
def answer_help(
    ...
    hr.status, hr.answered_by, hr.answer_text, hr.answered_at = (
        "answered",
        user.user_id,
        req.answer_text,
        utc_now(),
    ...
    record_change(
        db,
        "help_request.answered",
        "machine",
        hr.machine_id,
        {
            "request_id": hr.request_id,
            "operator_id": hr.operator_id,
            "content_id": hr.content_id,
            "answer_text": hr.answer_text,
            "answered_at": iso_ms(hr.answered_at),
        },
    )
    _resolve_group(db, f"help:{request_id}", user, "Answered")
    notify(db, machine.site_id, ["follow-ups"])
    db.commit()
