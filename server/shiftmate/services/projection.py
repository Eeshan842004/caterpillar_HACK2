...
entry is rejected on its own without failing the rest of the batch. Every projector returns
`(status, reason, keys)`; `keys` are the console invalidation keys (§6.3) published after the batch commits.
"""
...
from shiftmate.services.fleet_sim import apply_device_entry
...
FOLLOW_UP_REPORTS = {"alert_feedback", "help_request", "reassignment_request", "supervisor_notification"}

Projected = tuple[str, str | None, list[str]]  # (status, reason, invalidation keys)
...
def project_envelope(db: Session, device: Device, envelope: OutboxEnvelope) -> SyncPushResult:
    ...
    try:
        with db.begin_nested():
            status, reason, keys = _handle(db, device, ctx, envelope)
        notify(db, ctx.site_id, keys)
...
def _handle(db: Session, device: Device, ctx: MachineContext, env: OutboxEnvelope) -> Projected:
    body = env.body
    ...
    row = LedgerEntry(
        ...
        rule_or_model_version=entry.rule_or_model_version,
    ...
    db.add(row)
    db.flush()
    status, reason, keys = _project(db, device, ctx, row)
    if apply_device_entry(db, row):  # the device's own machine becomes a live fleet row (S6)
        keys = [*keys, "fleet"]
    return status, reason, keys


# ----------------------------------------------------------------------------------------------- projectors


def _project(db: Session, device: Device, ctx: MachineContext, row: LedgerEntry) -> Projected:
    ...
    if kind == "shift_event":
        if subtype == "start":
            db.merge(
                Shift(
                    shift_id=row.shift_id or row.entry_id,
    ...
    if (kind == "inference" and subtype == "finding") or (kind == "correction" and subtype == "finding"):
        _finding(db, ctx, row)
        return "confirmed", None, ["follow-ups"]

    if (
        kind == "incident"
        or (kind, subtype) in INCIDENT_FAMILY
        or (kind == "correction" and subtype in {"incident_report", "incident_extraction"})
    ):
        _incident(db, device, ctx, row)
        return "confirmed", None, ["incidents", "follow-ups"]
    ...
    if kind == "observation" and subtype == "signal_summary_5m":
        db.merge(
            MachineSummary(
                ...
                machine_id=row.machine_id,
                window_start=datetime.fromtimestamp(p["window_start"] / 1000.0, UTC).replace(tzinfo=None),
                window_end=datetime.fromtimestamp(p["window_end"] / 1000.0, UTC).replace(tzinfo=None),
                metrics=p,
            )
        )
        return "confirmed", None, []
    ...
    if kind == "handover_item" and subtype in {"acknowledged", "resolved"}:
        ...
        if item is not None:
            if subtype == "acknowledged":
                ...
                record_change(
                    db,
                    "handover_item.resolved",
                    "machine",
                    row.machine_id,
                    {
                        "item_id": item.item_id,
                        "resolved_by_role": "task_completed",
                        "resolved_at": _iso(row.observed_at),
                        "note": None,
                    },
                )
        return "confirmed", None, ["handovers"]

    return "confirmed", None, []  # everything else: mirror only (§8.21)


def _task_event(db: Session, ctx: MachineContext, row: LedgerEntry) -> Projected:
    p = row.payload
...
def _finding(db: Session, ctx: MachineContext, row: LedgerEntry) -> None:
    ...
    if finding is None or owner == "site":
        set_site_delay_contribution(
            db, root_entry_id=root, finding=finding, at=row.observed_at, zone_name=zone_name
        )
        return
...
def _incident(db: Session, device: Device, ctx: MachineContext, row: LedgerEntry) -> None:
    ...
    if row.kind == "incident" and row.subtype == "created":
        ...
        default_type = (
            "unsafe_condition" if any(t.startswith("A-BELT") for t in trigger_types) else "near_miss"
        )
    ...
    open_group(
        ...
        group_key=f"incident:{incident_id}",
    ...
    if inc.type == "near_miss" and inc.zone_id:
        ...
        count = (
            db.query(Incident)
            .filter(
                Incident.zone_id == inc.zone_id, Incident.type == "near_miss", Incident.occurred_at >= since
            )
            .count()
        )
        if count >= 2:
...
def _apply_chain_result(db: Session, ctx: MachineContext, device: Device, bad_seq: int | None) -> None:
    ...
    for inc in incidents:
        seqs = [
            r.chain_seq
            for r in db.query(LedgerEntry)
            .filter(LedgerEntry.device_id == device.device_id, LedgerEntry.chain_seq.isnot(None))
            .all()
    ...
    if bad_seq is not None:
        open_group(
            ...
            summary=f"Hash chain fails or has a gap at sequence {bad_seq}.",
            priority=1,
            at=utc_now(),
...
def _overspeed_zone(db: Session, ctx: MachineContext, row: LedgerEntry) -> None:
    ...
    rows = (
        ...
        .filter(
            LedgerEntry.kind == "alert", LedgerEntry.subtype == "raised", LedgerEntry.observed_at >= since
        )
        .all()
    )
    operators = {
    ...
    if len(operators) >= 3:
        open_group(
            db,
            site_id=ctx.site_id,
            category="overspeed_zone",
            group_key=f"overspeed_zone:{zone_id}",
            ...
            at=row.observed_at,
            zone_id=zone_id,
            metrics={"operators": len(operators)},
        )


def _report(db: Session, ctx: MachineContext, row: LedgerEntry) -> Projected:
    ...
    if subtype == "alert_feedback":
        open_group(
            ...
            summary=f"Operator marked alert {p['alert_id']} as {p['feedback']}.",
            priority=3,
            at=row.observed_at,
            machine_id=row.machine_id,
            related_id=p["alert_id"],
        )
        ...
        open_group(
            ...
            group_key=f"reassign:{row.entry_id}",
            title=f"Reassignment requested for {p['task_id']}",
            summary=f"Reason: {p['reason_code'].replace('_', ' ')}"
            + (f"; next task risk: {risk}" if risk else ""),
            priority=2,
            at=row.observed_at,
            machine_id=row.machine_id,
            related_id=row.entry_id,
        ...
        risk_text = {
            "likely_miss": "likely to miss its start window",
            "at_risk": "at risk of missing its start window",
            "none": "on time",
        ...
        open_group(
            db,
            site_id=ctx.site_id,
            category="supervisor_notification",
            group_key=f"notify:{row.entry_id}",
            ...
            priority=2 if risk == "likely_miss" else 3,
            at=row.observed_at,
            machine_id=row.machine_id,
            related_id=row.entry_id,
            metrics={"impact": impact},
        )
    return "confirmed", None, ["follow-ups"] if subtype in FOLLOW_UP_REPORTS else []


def _handover_bundle(db: Session, device: Device, ctx: MachineContext, env: OutboxEnvelope) -> Projected:
    body = env.body
    ...
    db.add(
        Handover(
            ...
            machine_id=device.machine_id,
            from_shift_id=handover.get("from_shift_id"),
            from_operator_id=handover.get("from_operator_id"),
            device_id=device.device_id,
            voice_note_upload_id=handover.get("voice_note_upload_id"),
            wording_method=handover.get("wording_method", "template"),
            created_at=parse_ts(handover.get("created_at")),
            data_origin="live",
        )
    )
    for item in items:
    ...
    db.flush()
    record_change(
        db, "handover.published", "machine", device.machine_id, {"handover": handover, "items": items}
    )
    return "confirmed", None, ["handovers"]
...
__all__ = ["FollowUp", "Machine", "Rejected", "Site", "parse_ts", "project_envelope", "wire_sha256"]
