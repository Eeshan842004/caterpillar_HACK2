...
from shiftmate.services.scenarios import draft_for_incident
...
def _summary(db: Session, inc: Incident) -> IncidentSummary:
    return IncidentSummary(
        ...
        site_id=inc.site_id,
        zone_name=_zone_name(db, inc.zone_id),
        occurred_at=iso_ms(inc.occurred_at),
        type=inc.type,
        severity=inc.severity,
        status=inc.status,
        origin=inc.origin,
        chain_ok=inc.chain_ok,
        sent_to_trainer=inc.sent_to_trainer,
...
def _detail(db: Session, inc: Incident, user: ConsoleUser) -> IncidentDetail:
    ...
    for e in (
        ...
        .filter(LedgerEntry.machine_id == inc.machine_id)
    ...
    return IncidentDetail(
        **_summary(db, inc).model_dump(),
        ...
        fields=inc.fields or {},
        snapshot=inc.snapshot,
        snapshot_complete=inc.snapshot_complete,
        history=history,
    )
...
def list_incidents(
    ...
    if cursor:
        ...
        q = q.filter(
            or_(
                Incident.occurred_at < occurred_dt,
    ...
    page = rows[:limit]
    nxt = (
        encode_cursor([page[-1].occurred_at.isoformat(), page[-1].incident_id]) if len(rows) > limit else None
    )
...
def review_incident(
        ...
        if name == "contact":
            if value not in ("yes", "no", "unknown"):
                raise ShiftMateException(
                    status_code=422, code="validation_error", message="contact must be yes/no/unknown"
                )
        elif name in REVIEWABLE:
            REVIEWABLE[name](value)  # raises ValueError on an invalid enum value
        else:
            raise ShiftMateException(
                status_code=422, code="validation_error", message=f"{name} cannot be corrected"
            )
    ...
    record_change(
        ...
        {
            "incident_id": inc.incident_id,
            "reviewed_at": iso_ms(inc.reviewed_at),
            "field_corrections": corrections,
        },
    )
    keys = ["incidents", "follow-ups"]
    if inc.type == "near_miss" and inc.scenario_id is None:  # §8.23: reviewed near miss → template draft
        draft_for_incident(db, inc, user.user_id)
        keys.append("scenarios")
    notify(db, inc.site_id, keys)
    db.commit()
...
def send_to_trainer(
    ...
    if inc.scenario_id is None:  # "creates a draft if missing" (§6.3)
        draft_for_incident(db, inc, user.user_id)
    notify(db, inc.site_id, ["incidents", "scenarios"])
    db.commit()
