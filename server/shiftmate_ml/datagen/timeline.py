...
SITE_DELAY_REASONS = {"waiting_truck", "waiting_loader", "shovel_queue", "crusher_queue", "access_blocked", "instructed_hold"}
...
SITE_DEFAULT_ZONE = {"SITE-CHN-01": "Z-CHN-TR1", "SITE-BLR-02": "Z-BLR-TR1", "SITE-MIN-03": "Z-MIN-YARD", "SITE-MIN-04": "Z-MIN4-R1"}

LESSONS = {
    "excavator": ["ex-l-belt-lockout", "ex-l-swing-radius", "ex-l-truck-loading", "ex-s-person-swing", "ex-s-truck-queue"],
...
def visibility_band(vis_m: Optional[float], darkness: bool) -> str:
...
def temperature_band(temp_c: Optional[float]) -> str:
...
class TimelineBuilder:
        ...
        *,
        ...
        if self._is_chain_member(kind, subtype, payload):
            ...
            canonical = canonical_json({
                "entry_id": eid, "device_id": dev_id, "shift_id": shift_id, "machine_id": machine_id,
                "operator_id": operator_id, "kind": kind, "subtype": subtype, "source": source,
                "payload": payload, "observed_at": epoch_ms(observed_at), "recorded_at": epoch_ms(rec_at),
                "supersedes": supersedes, "original_text": original_text,
            })
            content_hash = hashlib.sha256((chain["prev_hash"] + "\n" + canonical).encode("utf-8")).hexdigest()
            entry.update(chain_seq=chain["seq"], prev_hash=chain["prev_hash"], content_hash=content_hash,
    ...
    def _operator_for_day(self, machine: Dict[str, Any], day: datetime, day_idx: int) -> Optional[str]:
        hired = [
            op for op in self.machine_ops[machine["machine_id"]]
            if datetime.strptime(self.operators[op]["hired_at"], "%Y-%m-%d").date() <= day.date()
        ]
        if not hired:
            return None
    ...
    def _condition_at_local(self, site: Dict[str, Any], day: datetime, local_hour: int) -> Optional[Dict[str, Any]]:
        local = datetime(day.year, day.month, day.day, local_hour, tzinfo=timezone.utc)
        return self.conditions_map.get(f"{site['site_id']}:{iso_utc(local - timedelta(minutes=site['utc_offset_minutes']))}")

    def condition_at(self, site: Dict[str, Any], when_utc: datetime) -> Optional[Dict[str, Any]]:
    ...
    def _build_shift(self, m: Dict[str, Any], op_id: str, day: datetime, day_idx: int, week: int) -> None:
        ...
        shift_start = datetime(day.year, day.month, day.day, start_hour, tzinfo=timezone.utc) - offset
        shift_id = self.uid(f"shift:{mid}:{day_idx}")
        handover_id = self.uid(f"handover:{mid}:{day_idx}")
        hired = datetime.strptime(op["hired_at"], "%Y-%m-%d").date()
        exp_months = months_between(hired, day.date())

        self.ledger_entry(
            dev_id, shift_id, mid, op_id, "shift_event", "start", "reported",
            {"auth_method": "pin", "language": op["language"],
             "guidance": "guided" if (day.date() - hired).days < 30 else "concise",
             "profile_id": m["profile_id"], "profile_version": int(m.get("profile_version", 1))},
            shift_start, "site",
        )
        ...
        if state.open_handover_items and state.last_operator != op_id:
            for item in state.open_handover_items:
                self.ledger_entry(dev_id, shift_id, mid, op_id, "handover_item", "acknowledged", "reported",
                                  {"item_id": item["item_id"]}, shift_start + timedelta(minutes=3), "next_operator")
        ...
        if self.rng.random() < req.get("warmup_prob", 0.0):
            ...
            self._required_idle(dev_id, shift_id, m, op_id, site, None, SITE_DEFAULT_ZONE[site_id],
                                cursor, dur, basis="warmup")
        ...
        last_end = cursor
        for seq in (1, 2):
            result = self._build_task(m, profile, site, op, exp_months, dev_id, shift_id, day, week, seq, cursor)
            if result is None:
                continue
            ...
            if heavy and self.rng.random() < req.get("cooldown_prob", 0.0):
                ...
                self._required_idle(dev_id, shift_id, m, op_id, site, None, zone, task_end + timedelta(seconds=20),
                                    dur, basis="cooldown")
        ...
        if self.rng.random() < 0.15:
            ...
            note = self.ledger_entry(dev_id, shift_id, mid, op_id, "report", "handover_note", "reported",
                                     {"handover_id": handover_id, "item_id": item_id, "text": text, "via": "button"},
                                     t, "next_operator")
            self.ledger_entry(dev_id, shift_id, mid, op_id, "handover_item", "added", "reported",
                              {"handover_id": handover_id, "item_id": item_id, "item_type": "note", "text_key": None,
                               "text_params": None, "text": text, "audiences": ["next_operator"],
                               "source_entry_ids": [note["entry_id"]], "task_id": None, "incident_id": None,
                               "carried_from_item_id": None},
                              t + timedelta(seconds=30), "next_operator")
            state.open_handover_items.append({"item_id": item_id})
        state.last_operator = op_id

        self.ledger_entry(dev_id, shift_id, mid, op_id, "shift_event", "end", "reported",
                          {"handover_id": handover_ref}, shift_end, "site")
        self.shifts.append({
            "shift_id": shift_id, "site_id": site_id, "machine_id": mid, "operator_id": op_id,
            "started_at": iso_utc(shift_start), "ended_at": iso_utc(shift_end),
            "local_shift_date": day.strftime("%Y-%m-%d"), "shift_code": shift_code,
            "data_origin": self.origin, "recorded_at": iso_utc(shift_start),
        })
    ...
    def _pick_task(self, mclass: str, op_id: str) -> Tuple[str, str, float, str]:
        ...
        if mclass == "haul_truck":
            ...
            return tt, ("overburden" if tt == "haul_overburden" else "ore"), float(rng.integers(6, 24) * 90), "t"
        if op_id == DEMO_RAVI:
            ...
            tt = str(rng.choice(["trenching", "truck_loading", "backfilling", "grading", "pipe_lifting"],
                                p=[0.33, 0.33, 0.14, 0.12, 0.08]))
        if tt == "trenching":
            mat = "clay" if op_id == DEMO_RAVI else str(rng.choice(
        ...
        if tt in ("truck_loading", "backfilling"):
            return tt, str(rng.choice(["clay", "sand", "gravel", "rock"], p=[0.40, 0.25, 0.20, 0.15])), float(rng.integers(30, 110)), "m3"
        if tt == "grading":
            return tt, str(rng.choice(["clay", "sand", "gravel", "topsoil"], p=[0.35, 0.25, 0.25, 0.15])), float(rng.integers(150, 600)), "m2"
    ...
    def _build_task(self, m, profile, site, op, exp_months, dev_id, shift_id, day, week, seq, start_utc):
        ...
        planner = round(baseline * self.raw["planner"].get("scale", 0.92)
                        * math.exp(float(rng.normal(0, self.raw["planner"].get("sigma", 0.08)))), 1)
        rule_prefix = f"profile={m['profile_id']}@{m.get('profile_version', 1)}"

        common = {
            "task_id": task_id, "assignment_revision": 1, "site_id": site_id, "zone_id": zone,
            "machine_id": mid, "operator_id": op_id, "shift_id": shift_id, "task_type": tt, "material": mat,
            "quantity": qty, "unit": unit, "location_text": zone_name, "priority": 1 if seq == 1 else 2,
            "completion_criterion": f"{tt.replace('_', ' ').capitalize()} {qty:g} {unit} at {zone_name}",
            "assignment_source": "dispatcher", "planned_date": day.strftime("%Y-%m-%d"), "sequence": seq,
            "planned_start_at": iso_utc(planned_start) if planned_start else None,
            "planned_start_window_min": window_min, "estimate_requested_at": iso_utc(estimate_at),
            "planner_minutes": planner, "profile_id": m["profile_id"], "profile_version": int(m.get("profile_version", 1)),
            "skill_level_at_start": op["skill_level"], "experience_months_at_start": exp_months,
            "machine_age_years_at_start": machine_age, "condition_id": cond["condition_id"] if cond else None,
            "weather_at_start": weather, "visibility_at_start": visibility, "temperature_band_at_start": temp_band,
            "time_of_day_at_start": tod, "site_congestion_at_start": congestion, "darkness_at_start": dark,
            "baseline_minutes": baseline, "data_origin": self.origin,
        }
        ...
        if op_id != DEMO_RAVI and rng.random() < self.raw.get("task_events", {}).get("cancel_prob", 0.0):
            self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "cancel", "reviewed",
                              {"task_id": task_id, "assignment_revision": 1, "reason_code": None, "output_qty": None,
                               "actual_start": None, "actual_end": None, "active_min": None, "waiting_min": None,
                               "break_min": None, "paused_min": None},
                              start_utc, "site", rule="supervisor_reassignment@1")
            self.tasks.append({**common, "status": "CANCELLED", "actual_start_at": None, "actual_end_at": None,
                               "actual_active_min": None, "actual_waiting_min": None, "actual_break_min": None,
                               "actual_paused_min": None, "actual_elapsed_min": None, "output_qty": None,
                               "completion_quality": "not_applicable", "training_eligible": False,
                               "training_exclusion_reason": "cancelled", "split_temporal": "excluded",
                               "split_unseen_operator": "excluded", "recorded_at": iso_utc(start_utc)})
            return None
        ...
        if cond and (mid, cond["condition_id"]) not in self.forecast_logged:
            ...
            issued = datetime.fromisoformat(cond["issued_at"].replace("Z", "+00:00"))
            self.ledger_entry(dev_id, shift_id, mid, op_id, "observation", "condition_forecast", "observed",
                              {"site_id": site_id, "valid_from": cond["valid_from"], "valid_to": cond["valid_to"],
                               "weather": cond["weather"], "visibility": cond["visibility"],
                               "visibility_m": cond["visibility_m"], "temp_c": cond["temp_c"],
                               "heat_index_c": cond["heat_index_c"], "wind_kmh": cond["wind_kmh"],
                               "precipitation_mm": cond["precipitation_mm"], "issued_at": cond["issued_at"],
                               "source": "seed"},
                              issued, "site", freshness_s=None)

        # Active work minutes from the effect model (§8.22)
        effects = self.cfg.effects
        mult, _ = compute_task_effects(
            effects, skill_level=op["skill_level"], experience_months=exp_months, weather=weather or "clear",
            visibility=visibility or "good", temp_band=temp_band or "mild", time_of_day=tod, congestion=congestion,
            machine_age_years=machine_age, material=mat, is_wind_sensitive=bool(tt_cfg.get("wind_sensitive")),
            task_type=tt, operator_effect=self.gen.operator_effects[op_id], site_effect=self.gen.site_effects[site_id],
            noise=sample_task_noise(rng, baseline, effects.get("noise_variance", {})), effect_scale=self.scale,
        )
        work_min = baseline * mult * math.exp(
            self.raw.get("calibration", {}).get("log_intercept_by_class", {}).get(mclass, 0.0))
        ...
        self.ledger_entry(dev_id, shift_id, mid, op_id, "inference", "estimate", "inferred",
                          {"task_id": task_id, "basis": "fallback", "baseline_min": baseline,
                           "p10_min": round(baseline * 0.8, 1), "p50_min": round(baseline, 1),
                           "p90_min": round(baseline * 1.35, 1),
                           "expected_wait_min": float(tt_cfg.get("default_expected_wait_min", 5)),
                           "factors": [], "artifact_id": "baseline-fallback@1", "personal_offset": None,
                           "context": {"weather": weather, "visibility": visibility, "temperature_band": temp_band,
                                       "time_of_day": tod, "site_congestion": congestion, "darkness": dark}},
                          estimate_at, "site", rule=f"estimate@1;{rule_prefix}", confidence="medium")
        self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "start", "reported",
                          self._task_payload(task_id), start_utc, "site")
        ...
        for (s, e, spec) in timeline:
            ...
            if spec["type"] == "pause":
                ...
                self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "pause", "reported",
                                  self._task_payload(task_id), s, "site")
                self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "resume", "reported",
                                  self._task_payload(task_id), e, "site")
            elif spec["type"] == "block":
                waiting += minutes
                intervals.append(Interval("block", s, e))
                self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "block", "reported",
                                  {**self._task_payload(task_id), "reason_code": spec["reason"]}, s, "site")
                self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "resume", "reported",
                                  self._task_payload(task_id), e, "site")
            else:
        ...
        self.ledger_entry(dev_id, shift_id, mid, op_id, "task_event", "complete", "reported",
                          {"task_id": task_id, "assignment_revision": 1, "reason_code": None, "output_qty": output,
                           "actual_start": iso_utc(start_utc), "actual_end": iso_utc(end_utc), "active_min": active,
                           "waiting_min": waiting, "break_min": brk, "paused_min": paused},
                          end_utc, "site")
        ...
        self.tasks.append({
            **common, "status": "COMPLETED", "actual_start_at": iso_utc(start_utc), "actual_end_at": iso_utc(end_utc),
            "actual_active_min": active, "actual_waiting_min": waiting, "actual_break_min": brk,
            "actual_paused_min": paused, "actual_elapsed_min": elapsed, "output_qty": output,
            "completion_quality": "partial" if partial else "confirmed", "training_eligible": eligible,
            "training_exclusion_reason": "partial_completion" if partial else None,
            "split_temporal": split_a, "split_unseen_operator": split_b, "recorded_at": iso_utc(end_utc),
        })
        ...
        self._summaries(dev_id, shift_id, m, op_id, site, task_id, start_utc, end_utc, intervals, load_pct, rule_prefix)
        self._alerts(dev_id, shift_id, m, op_id, op, site, task_id, zone, start_utc, end_utc, weather, dark, rule_prefix)
        return end_utc, heavy_load and load_pct >= self.raw.get("required_idle", {}).get("cooldown_load_pct", 60), zone

    @staticmethod
    def _task_payload(task_id: str) -> Dict[str, Any]:
        return {"task_id": task_id, "assignment_revision": 1, "reason_code": None, "output_qty": None,
                "actual_start": None, "actual_end": None, "active_min": None, "waiting_min": None,
                "break_min": None, "paused_min": None}
    ...
    def _sample_interruptions(self, tt, mclass, op_id, congestion, weather) -> List[Dict[str, Any]]:
        ...
        for _ in range(n_waits):
            minutes = float(np.clip(rng.lognormal(math.log(waits["duration_lognormal_median_min"]),
                                                  waits["duration_lognormal_sigma"]), 2.0, 45.0))
            unexplained = rng.random() >= waits.get("reported_prob", 0.85) or (
                op_id in self.gen.habitual_unexplained_ops and rng.random() < 0.3)
            spec = {"type": "idle", "minutes": round(minutes, 1),
        ...
        if rng.random() < te.get("block_prob", 0.0):
            ...
            out.append({"type": "block", "minutes": round(float(rng.uniform(lo, hi)), 1),
                        "reason": str(rng.choice(["access_blocked", "utility_mark", "waiting_instruction", "weather"]))})
        rng.shuffle(out)
        return out

    # ------------------------------------------------------------------ idle

    def _idle(self, dev_id, shift_id, m, op_id, site, task_id, zone, s, e, spec) -> Optional[str]:
        ...
        self.ledger_entry(dev_id, shift_id, mid, op_id, "idle_event", "started", "observed",
                          {"idle_event_id": idle_id, "candidate_started_at": iso_utc(s), "task_id": task_id, "zone_id": zone},
                          s, "site")
        ...
        if reason:
            ...
            reason_at = prompted + timedelta(seconds=int(rng.integers(5, 40))) if prompted else s + timedelta(
                seconds=int(min(dur - 5, max(20, dur // 2))))
            report_entry = self.ledger_entry(dev_id, shift_id, mid, op_id, "report", "idle_reason", "reported",
                                             {"idle_event_id": idle_id, "reason_code": reason, "free_text": None,
            ...
            if reason in SITE_DELAY_REASONS and rng.random() < self.raw["waits"].get("correction_prob", 0.0):
                options = [r for r in sorted(SITE_DELAY_REASONS) if r != reason and r in self._machine_reasons(m)]
                corrected = str(rng.choice(options)) if options else None
        effective = corrected or reason
        if effective:
            category = "site_delay" if effective in SITE_DELAY_REASONS else ("break" if effective == "break" else "other")

        self.ledger_entry(dev_id, shift_id, mid, op_id, "idle_event", "ended", "observed",
                          {"idle_event_id": idle_id, "ended_at": iso_utc(e), "duration_s": dur, "required_s": 0},
                          e, "site")
        idle_class = "reported" if reason else "unexplained"
        self.ledger_entry(dev_id, shift_id, mid, op_id, "inference", "idle_classification", "inferred",
                          {"idle_event_id": idle_id, "idle_class": idle_class, "required_s": 0,
                           "non_required_s": dur, "category": category},
                          e, "site", rule=f"idle@1;profile={m['profile_id']}@{m.get('profile_version', 1)}",
                          confidence="high")

        finding = None
        if reason in SITE_DELAY_REASONS:
            finding = self._site_delay_finding(dev_id, shift_id, m, op_id, site, idle_id, zone, reason, dur, e,
                                               report_entry["entry_id"])
        if corrected and report_entry is not None:
            fix_at = e + timedelta(minutes=float(rng.uniform(2, 40)))
            self.ledger_entry(dev_id, shift_id, mid, op_id, "correction", "idle_reason", "reported",
                              {"target_kind": "report", "target_subtype": "idle_reason",
                               "replacement": {"idle_event_id": idle_id, "reason_code": corrected, "free_text": None,
                                               "via": "voice"}},
                              fix_at, "site", supersedes=report_entry["entry_id"],
                              original_text=f"actually, {corrected.replace('_', ' ')}")
            if finding is not None:
                replacement = dict(finding["payload"], reason_code=corrected, possible_explanations=[corrected])
                self.ledger_entry(dev_id, shift_id, mid, op_id, "correction", "finding", "inferred",
                                  {"target_kind": "inference", "target_subtype": "finding", "replacement": replacement},
                                  fix_at + timedelta(seconds=1), "site", supersedes=finding["entry_id"],
                                  rule="findings@1")

        self.idles.append({
            "idle_event_id": idle_id, "site_id": site["site_id"], "machine_id": mid, "operator_id": op_id,
            "shift_id": shift_id, "task_id": task_id, "zone_id": zone, "started_at": iso_utc(s), "ended_at": iso_utc(e),
            "recorded_at": iso_utc(e), "duration_s": dur, "required_s": 0, "non_required_s": dur,
            "required_basis": "none", "idle_class": idle_class, "reason_code": effective,
            "reason_entry_id": report_entry["entry_id"] if report_entry else None,
            "reason_recorded_at": iso_utc(reason_at) if reason_at else None, "category": category,
            "prompted_at": iso_utc(prompted) if prompted else None,
            "prompt_answered_at": iso_utc(reason_at) if (prompted and reason_at and reason_at >= prompted) else None,
            "sensor_coverage_pct": 100.0, "evidence_status": "reported" if reason else "unresolved",
            "data_origin": self.origin,
        })
        return category
    ...
    def _site_delay_finding(self, dev_id, shift_id, m, op_id, site, idle_id, zone, reason, dur, at, report_id):
        local_date = (at + timedelta(minutes=site["utc_offset_minutes"])).strftime("%Y-%m-%d")
        payload = {
            "finding_id": self.uid("finding"), "subject_id": idle_id, "pattern_code": "idle_reported_wait",
            "owner": "site", "evidence_status": "reported", "reason_key": "finding.idle_reported_wait",
            "observed": {"text_key": "finding.observed.idle_wait", "params": {"minutes": round(dur / 60, 1)}},
            "possible_explanations": [reason], "related_entry_ids": [report_id], "machine_id": m["machine_id"],
            "site_id": site["site_id"], "zone_id": zone, "reason_code": reason, "minutes": round(dur / 60, 1),
            "local_date": local_date, "rule_version": "findings@1",
        }
        return self.ledger_entry(dev_id, shift_id, m["machine_id"], op_id, "inference", "finding", "inferred", payload,
                                 at + timedelta(seconds=2), "site", rule="findings@1", confidence="high")
    ...
    def _required_idle(self, dev_id, shift_id, m, op_id, site, task_id, zone, s, dur, basis):
        ...
        self.ledger_entry(dev_id, shift_id, mid, op_id, "idle_event", "ended", "observed",
                          ...
                          e, "site")
        self.ledger_entry(dev_id, shift_id, mid, op_id, "inference", "idle_classification", "inferred",
                          {"idle_event_id": idle_id, "idle_class": "required", "required_s": required,
                           "non_required_s": non_required, "category": None},
                          e, "site", rule=f"idle@1;profile={m['profile_id']}@{m.get('profile_version', 1)}",
                          confidence="high")
        self.idles.append({
            "idle_event_id": idle_id, "site_id": site["site_id"], "machine_id": mid, "operator_id": op_id,
            "shift_id": shift_id, "task_id": task_id, "zone_id": zone, "started_at": iso_utc(s), "ended_at": iso_utc(e),
            "recorded_at": iso_utc(e), "duration_s": dur, "required_s": required, "non_required_s": non_required,
            "required_basis": basis, "idle_class": "required", "reason_code": None, "reason_entry_id": None,
            "reason_recorded_at": None, "category": None, "prompted_at": None, "prompt_answered_at": None,
            "sensor_coverage_pct": 100.0, "evidence_status": "corroborated", "data_origin": self.origin,
        })

    # ------------------------------------------------------------------ summaries

    def _summaries(self, dev_id, shift_id, m, op_id, site, task_id, start, end, intervals, load_pct, rule_prefix):
        rng = self.rng
        ...
        while cursor < end:
            ...
            fuel = round((working + travelling) / 3600 * burn_lph + idle_total / 3600 * (3.5 if mclass == "excavator" else 12.0), 3)
            if mclass == "excavator":
                cycles = int(round(working / 3600 * 150 * rng.uniform(0.85, 1.1)))
                max_speed = round(float(rng.uniform(0.5, 3.5)), 1)
            else:
                cycles = int(rng.random() < win / 720)
                max_speed = round(float(rng.uniform(18.0, 34.0)), 1)
            payload = {
                "window_start": epoch_ms(cursor), "window_end": epoch_ms(w_end), "engine_on_s": win,
                "secured_s": secured_s, "ready_s": ready_s, "working_s": working, "travelling_s": travelling,
                "unknown_s": unknown, "idle_s": idle_total,
                "fuel_used_l": None if "fuel_used_l" in missing else fuel,
                "load_cycles": cycles,
                "max_speed_kmh": None if "ground_speed_kmh" in missing else max_speed,
                "avg_load_factor_pct": None if "load_factor_pct" in missing else round(load_pct + float(rng.normal(0, 4)), 1),
                "belt_unfastened_moving_s": None if "seatbelt_fastened" in missing else 0,
                "samples": win - missing_s, "missing_samples": missing_s, "missing_signal_names": missing,
            }
            entry = self.ledger_entry(dev_id, shift_id, mid, op_id, "observation", "signal_summary_5m", "observed",
                                      payload, w_end, "site", freshness_s=0.0)
            self.summaries.append({
                "summary_id": entry["entry_id"], "site_id": site["site_id"], "machine_id": mid, "shift_id": shift_id,
                "task_id": task_id, "window_start": iso_utc(cursor), "window_end": iso_utc(w_end), "recorded_at": iso_utc(w_end),
                **{k: payload[k] for k in ("engine_on_s", "secured_s", "ready_s", "working_s", "travelling_s", "unknown_s",
                                           "idle_s", "fuel_used_l", "load_cycles", "max_speed_kmh", "avg_load_factor_pct",
                                           "belt_unfastened_moving_s", "samples", "missing_samples")},
                "missing_signal_names": json.dumps(missing), "profile_id": m["profile_id"],
                "profile_version": int(m.get("profile_version", 1)), "data_origin": self.origin,
            })
            cursor = w_end

    # ------------------------------------------------------------------ alerts & incidents

    def _alert_rows(self, dev_id, shift_id, m, op_id, site, task_id, zone, alert: Dict[str, Any],
                    transitions: List[Tuple[str, datetime, Optional[str]]], rule: str) -> None:
        for transition, at, clear_reason in transitions:
            source = "reported" if transition == "acknowledged" else "observed"
            payload = {
                "alert_id": alert["alert_id"], "alert_type": alert["alert_type"], "level": alert["level"],
                "group_key": alert["group_key"], "zone_id": zone, "object_id": alert.get("object_id"),
                "object_type": alert.get("object_type"), "place": alert.get("place"),
                "distance_m": alert.get("distance_m"), "ttc_s": alert.get("ttc_s"),
                "multiplier": alert.get("multiplier"), "occurrences": 1,
                "clear_reason": clear_reason if transition == "cleared" else None,
                "safe_exit": alert.get("safe_exit"),
            }
            entry = self.ledger_entry(dev_id, shift_id, m["machine_id"], op_id, "alert", transition, source, payload, at,
                                      "site", rule=rule)
            self.alerts.append({
                "alert_event_id": entry["entry_id"], "alert_id": alert["alert_id"], "site_id": site["site_id"],
                "machine_id": m["machine_id"], "shift_id": shift_id, "task_id": task_id, "zone_id": zone,
                "transition": transition, "alert_type": alert["alert_type"], "level": alert["level"],
                "group_key": alert["group_key"], "occurrences": 1, "object_id": alert.get("object_id"),
                "object_type": alert.get("object_type"), "place": alert.get("place"),
                "distance_m": alert.get("distance_m"), "ttc_s": alert.get("ttc_s"), "multiplier": alert.get("multiplier"),
                "clear_reason": payload["clear_reason"],
                "exit_cue": (alert.get("safe_exit") or {}).get("exit_cue"),
                "safe_exit_json": json.dumps(alert["safe_exit"], sort_keys=True) if alert.get("safe_exit") else None,
                "observed_at": iso_utc(at), "recorded_at": iso_utc(at), "source": source,
                "rule_or_model_version": rule, "profile_id": m["profile_id"],
                "profile_version": int(m.get("profile_version", 1)), "data_origin": self.origin,
            })

    def _alerts(self, dev_id, shift_id, m, op_id, op, site, task_id, zone, start, end, weather, dark, rule_prefix):
        rng = self.rng
        ...
        if rng.random() < (0.08 if op["skill_level"] == "beginner" else 0.02):
            t0 = at()
            alert = {"alert_id": self.uid("alert"), "alert_type": "A-BELT-OPER", "level": "WARNING", "group_key": "belt"}
            tr = [("raised", t0, None)]
            if rng.random() < 0.6:
            ...
            self._alert_rows(dev_id, shift_id, m, op_id, site, task_id, zone, alert, tr, f"seatbelt@1;{rule_prefix}")

        # Overspeed on haul roads — A-SPEED WARNING (3 habitual operators)
        if mclass == "haul_truck" and rng.random() < (0.12 if op_id in self.gen.habitual_overspeed_ops else 0.02):
            t0 = at()
            alert = {"alert_id": self.uid("alert"), "alert_type": "A-SPEED", "level": "WARNING", "group_key": "speed"}
            self._alert_rows(dev_id, shift_id, m, op_id, site, task_id, zone, alert,
                             [("raised", t0, None), ("cleared", t0 + timedelta(seconds=float(rng.uniform(8, 25))), "below_limit")],
        ...
        if rng.random() < 0.06:
            ...
            obj_id = f"P{int(rng.integers(1, 99))}" if obj_type == "person" else f"V{int(rng.integers(1, 99))}"
            mult = 1.0
            if weather == "rain":
                mult *= 1.25
            if weather == "dusty":
            ...
            alert = {
                "alert_id": self.uid("alert"), "alert_type": "A-PROX-WARN" if warning else "A-PROX-CAUT",
                "level": "WARNING" if warning else "CAUTION", "group_key": f"prox:{obj_id}", "object_id": obj_id,
                "object_type": obj_type, "place": place,
                "distance_m": round(float(rng.uniform(3.0, 4.4) if warning else rng.uniform(5.0, 9.0)) * mult, 1),
                "ttc_s": round(float(rng.uniform(1.5, 2.9) if warning else rng.uniform(3.2, 5.8)), 1), "multiplier": mult,
            }
            t_clear = t0 + timedelta(seconds=float(rng.uniform(6, 30)))
            self._alert_rows(dev_id, shift_id, m, op_id, site, task_id, zone, alert,
                             [("raised", t0, None), ("cleared", t_clear, "object_clear")], f"proximity@1;{rule_prefix}")
            if warning:
        ...
        if rng.random() < 0.004:
            ...
            safe_exit = {
                "belt_transition_at": iso_utc(t0 - timedelta(seconds=float(rng.uniform(1, 9)))), "exit_cue": cue,
                "inputs": {"seatbelt_fastened": {"value": False, "fresh": True},
            ...
            alert = {"alert_id": self.uid("alert"), "alert_type": "A-EXIT-UNSEC", "level": "ADVISORY",
                     "group_key": "safe_exit", "safe_exit": safe_exit}
            self._alert_rows(dev_id, shift_id, m, op_id, site, task_id, zone, alert,
                             [("raised", t0, None), ("cleared", t0 + timedelta(seconds=float(rng.uniform(8, 40))), "secured")],
    ...
    def _incident(self, dev_id, shift_id, m, op_id, site, zone, task_id, alert, t0, task_end, weather, dark):
        ...
        pre = [{"ts": epoch_ms(t0 - timedelta(seconds=60 - i * 6)), "state": "WORKING", "speed_kmh": 2.0,
                "belt": True, "secure": False, "lat": lat, "lon": lon, "heading_deg": 90, "load_factor_pct": 55}
               for i in range(10)]
        snapshot = {"pre_s": 60, "post_s": 30, "samples": pre,
                    "proximity": [{"ts": epoch_ms(t0), "object_id": alert["object_id"], "type": alert["object_type"],
                                   "bearing_deg": PLACES.index(alert["place"]) * 45, "distance_m": alert["distance_m"],
                                   "closing_mps": 1.1}],
                    "alerts": [{"ts": epoch_ms(t0), "alert_type": alert["alert_type"], "level": alert["level"]}]}
        created = {
            "incident_id": incident_id, "origin": "auto", "trigger_alert_id": alert["alert_id"],
            "occurred_at": iso_utc(t0), "zone_id": zone,
            "observed": {"machine_state": "WORKING", "speed_kmh": 2.0, "belt_fastened": True, "secure_engaged": False,
                         "lat": lat, "lon": lon,
                         "detected": [{"object_id": alert["object_id"], "type": alert["object_type"],
                                       "place": alert["place"], "min_distance_m": alert["distance_m"]}],
                         "conditions": {"rain": weather == "rain", "dust": weather == "dusty", "darkness": dark},
                         "active_task_id": task_id},
            "snapshot": snapshot, "snapshot_complete": False,
            "severity_default": "high" if alert["object_type"] == "person" else "medium",
        }
        self.ledger_entry(dev_id, shift_id, mid, op_id, "incident", "created", "observed", created, t0, "safety")
        post = [{"ts": epoch_ms(t0 + timedelta(seconds=i * 6)), "state": "WORKING", "speed_kmh": 1.0, "belt": True,
                 "secure": False, "lat": lat, "lon": lon, "heading_deg": 90, "load_factor_pct": 50} for i in range(1, 6)]
        self.ledger_entry(dev_id, shift_id, mid, op_id, "incident", "snapshot_completed", "observed",
                          {"incident_id": incident_id, "post_samples": post, "truncated": False},
                          t0 + timedelta(seconds=30), "safety")
        if rng.random() < 0.7:
            at = task_end + timedelta(minutes=float(rng.uniform(1, 6)))
            fields = {"type": "near_miss", "object": alert["object_type"], "place": alert["place"], "contact": "no"}
            self.ledger_entry(dev_id, shift_id, mid, op_id, "report", "incident_report", "reported",
                              {"incident_id": incident_id, **fields, "severity": created["severity_default"], "via": "voice"},
                              at, "safety", original_text=f"near miss, {alert['object_type'].replace('_', ' ')} {alert['place'].replace('_', ' ')}, no contact")
            self.ledger_entry(dev_id, shift_id, mid, op_id, "inference", "incident_extraction", "inferred",
                              {"incident_id": incident_id, "method": "rules", "fields": fields, "no_incident": False},
                              at + timedelta(seconds=1), "safety", rule="incident_rules@1", confidence="high")
            self.ledger_entry(dev_id, shift_id, mid, op_id, "incident", "status_changed", "reported",
                              {"incident_id": incident_id, "status": "reported"}, at + timedelta(seconds=2), "safety")
    ...
    def _learning(self, dev_id, shift_id, mid, op_id, mclass, at):
        ...
        self.ledger_entry(dev_id, shift_id, mid, op_id, "inference", "recommendation", "inferred",
                          {"rec_id": self.uid("rec"), "content_id": content, "source": "task_prep", "pattern_code": None,
                           "condition": None, "reason_key": "rec.reason.task_prep", "reason_params": {}},
                          at, "operator_only", rule="recommender@1", confidence="medium")
        base = {"content_id": content, "mode": "full", "question_index": None, "choice_index": None, "correct": None,
                "refresher_opt_in": None, "review_step": None}
        self.ledger_entry(dev_id, shift_id, mid, op_id, "learning_event", "started", "reported", base,
                          at + timedelta(minutes=1), "operator_only")
        correct = bool(rng.random() < 0.75)
        self.ledger_entry(dev_id, shift_id, mid, op_id, "learning_event", "answered", "reported",
                          {**base, "question_index": 0, "choice_index": 0 if correct else 1, "correct": correct},
                          at + timedelta(minutes=2), "operator_only")
        self.ledger_entry(dev_id, shift_id, mid, op_id, "learning_event", "completed", "reported",
                          {**base, "refresher_opt_in": bool(rng.random() < 0.6)}, at + timedelta(minutes=3),
                          "operator_only")
