...
def test_push_rules_and_idempotency(client):
    ...
    idle = entry(device_id, "EX-07", "report", "idle_reason", "reported",
                 {"idle_event_id": str(uuid.uuid4()), "reason_code": "waiting_truck", "free_text": None, "via": "button"})
    start = entry(device_id, "EX-07", "task_event", "start", "reported",
                  {"task_id": "T-20260923-EX07-1", "assignment_revision": 1})
    private = entry(device_id, "EX-07", "learning_event", "completed", "reported",
                    {"content_id": "ex-l-belt-lockout"}, audience="operator_only")
    other_machine = entry(device_id, "HT-03", "task_event", "start", "reported",
                          {"task_id": "T-20260923-HT03-1", "assignment_revision": 1})
    bad_payload = entry(device_id, "EX-07", "report", "idle_reason", "reported",
                        {"idle_event_id": "x", "reason_code": "coffee"})
    no_rule = entry(device_id, "EX-07", "alert", "raised", "observed",
                    {"alert_id": "a1", "alert_type": "A-BELT-OPER", "level": "WARNING", "group_key": "belt"})
    bad_time = entry(device_id, "EX-07", "shift_event", "end", "reported", {"handover_id": None}, at="yesterday")
    server_only = entry(device_id, "EX-07", "incident", "reviewed", "reviewed", {"incident_id": "i1"}, audience="safety")

    results = push(client, device_id, secret, [idle, start, private, other_machine, bad_payload, no_rule, bad_time,
                                               server_only])
    ...
    assert push(client, device_id, secret, [changed])[idle["entry_id"]]["reason"] == "id_reuse_different_payload"
...
def test_one_bad_entry_does_not_fail_the_batch(client):
    ...
    unknown_task = entry(device_id, "EX-07", "report", "reassignment_request", "reported",
...
def test_handover_bundle_is_stored(client):
    ...
    batch = {"batch": [{"entry_id": handover_id, "type": "handover", "created_at": now_iso(), "body": {
        "kind": "handover_bundle",
        "handover": {"handover_id": handover_id, "machine_id": "EX-07", "from_shift_id": None,
                     "from_operator_id": "OP-0007", "created_at": now_iso(), "voice_note_upload_id": None,
                     "wording_method": "template"},
        "items": [{"item_id": str(uuid.uuid4()), "item_type": "note", "text": "Soft ground east edge",
                   "audiences": ["next_operator"], "source_entry_ids": []}]}}]}
    ...
    second = client.post("/api/v1/sync/push", content=body,
                         headers={**make_device_auth_headers("POST", "/api/v1/sync/push", body, device_id, secret),
                                  "Content-Type": "application/json"}).json()["results"][0]
