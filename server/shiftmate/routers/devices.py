...
from shiftmate.services.forecast import forecast_hour
from shiftmate.services.machine_context import machine_context
from shiftmate.services.scenarios import published_scenario
...
def _usable_code(db: Session, code: str, machine_id: str) -> PairingCode:
    ...
    if row is None or row.expires_at < utc_now() or row.machine_id != machine_id:
        raise ShiftMateException(
            status_code=404, code="pairing_code_invalid", message=f"Code not valid for {machine_id}."
        )
    ...
    if not row.reusable and row.used_at is not None:
        raise ShiftMateException(
            status_code=409, code="pairing_code_used", message="This pairing code has already been used."
        )
    return row
...
def _pair_response(db: Session, device: Device) -> DevicePairResponse:
    ...
    return DevicePairResponse(
        ...
        device_secret=derive_device_secret(device.device_id),
        machine_id=ctx.machine_id,
        site_id=ctx.site_id,
        profile_id=ctx.profile_id,
        profile_version=ctx.profile_version,
        server_time=iso_ms(utc_now()),
    )
...
def pair_device(req: DevicePairRequest, request: Request, db: Session = Depends(get_db)):
    ...
    if db.get(Machine, req.machine_id) is None:
        raise ShiftMateException(
            status_code=404, code="not_found", message=f"Machine {req.machine_id} was not found."
        )
    ...
    db.add(
        AuditLog(
            ...
            target_type="device",
            target_id=device_id,
            detail={"machine_id": req.machine_id},
            created_at=now,
        )
    )
    db.commit()
...
def rebind_device(
    req: DeviceRebindRequest, db: Session = Depends(get_db), device: Device = Depends(verify_device_auth)
):
    code_row = _usable_code(db, req.pairing_code, req.machine_id)
    now = utc_now()
    device.machine_id = req.machine_id
...
def get_bootstrap(db: Session = Depends(get_db), device: Device = Depends(verify_device_auth)):
    ...
    tasks = (
        ...
        .filter(
            TaskAssignment.machine_id == device.machine_id,
    ...
    for ho in (
        db.query(Handover)
        .filter(Handover.machine_id == device.machine_id)
        .order_by(Handover.created_at.desc())
        .all()
    ):
        items = (
            db.query(HandoverItem)
            .filter(HandoverItem.handover_id == ho.handover_id, HandoverItem.status == "open")
            .all()
        )
        if not items:
            continue
        handovers.append(
            {
                ...
                "created_at": iso_ms(ho.created_at),
    ...
    forecast_rows = (
        db.query(Forecast)
        .filter(
            Forecast.site_id == ctx.site_id,
            Forecast.valid_to >= now - timedelta(hours=12),
    ...
    scenarios = [
        published_scenario(s)
        for s in db.query(Scenario)
        .filter(Scenario.status == "approved", Scenario.machine_class == ctx.machine_class)
        .all()
    ]
    artifacts = [
        a.body
        for a in db.query(ModelArtifact)
        .filter((ModelArtifact.machine_class == ctx.machine_class) | (ModelArtifact.kind == "intent"))
        .all()
    ]
    ...
    return DeviceBootstrapResponse(
        ...
        site=SiteWire(
            ...
            lat=site.lat,
            lon=site.lon,
            utc_offset_minutes=site.utc_offset_minutes,
            diesel_price_inr_per_l=site.diesel_price_inr_per_l,
            dark_start_local=site.dark_start_local,
            dark_end_local=site.dark_end_local,
            job_efficiency_override=site.job_efficiency_override,
            congestion_level=site.congestion_level,
        ),
        zones=[
            ZoneWire(
                zone_id=z.zone_id,
        ...
        operators=[
            OperatorWire(
                ...
                hired_at=o.hired_at.isoformat(),
                site_id=o.site_id,
                pin_salt=o.pin_salt,
                pin_hash=o.pin_hash,
                pin_iterations=o.pin_iterations,
            )
            for o in operators
        ],
        assignments=[
        ...
        forecast={
            "site_id": ctx.site_id,
            "hours": [forecast_hour(f) for f in forecast_rows],
        },
    )


__all__ = ["datetime", "router"]
