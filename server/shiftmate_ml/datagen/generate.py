...
def generate_assumptions_doc(config: GenConfig, output_path: Path = ASSUMPTIONS_PATH) -> str:
    ...
    lines.extend(
        [
            ...
            (
                "- **Roster:** every machine class has beginners, intermediates and experts with overlapping experience; "
                "new hires only work after their hire date; experience is computed per task date."
            ),
            "",
            "## 4. Sensitivity Runs",
            "",
...
class SyntheticGenerator:
    def __init__(self, config: GenConfig, effect_scale: float = 1.0, out_dir: Path | None = None):
        self.config = config
        ...
        self.working_dates = self._compute_working_dates(
            config.anchor_date, config.num_weeks * config.working_days_per_week
        )
        ...
        self.site_effects = {
            s["site_id"]: float(
                self.rng.normal(0, config.effects.get("site_variance", {}).get("sigma", 0.04))
            )
            for s in self.sites
        }
        self.operator_effects = {
            o["operator_id"]: float(
                self.rng.normal(0, config.effects.get("operator_variance", {}).get("sigma", 0.06))
            )
        ...
        ht_op_ids = [
            o["operator_id"] for o in self.operators if o.get("regular_machine_class") == "haul_truck"
        ]
        self.habitual_overspeed_ops = set(self.rng.choice(ht_op_ids, size=3, replace=False))

        # Fixed synthetic device UUID per machine
        self.device_ids = {
            m["machine_id"]: deterministic_uuid(self.rng, f"dev:{m['machine_id']}") for m in self.all_machines
        }
    ...
    def _generate_conditions(self) -> list[dict[str, Any]]:
        ...
        for site in self.sites:
            ...
            for date in calendar:
                ...
                issued_local = datetime(prev_date.year, prev_date.month, prev_date.day, 18, 0, 0, tzinfo=UTC)
                ...
                for h in range(24):
                    valid_from_local = datetime(date.year, date.month, date.day, h, 0, 0, tzinfo=UTC)
                    ...
                    if wx == "foggy":
                        ...
                        vis_m = (
                            float(self.rng.uniform(1200, 3500))
                            if self.rng.random() < 0.75
                            else float(self.rng.uniform(600, 950))
                        )
                    elif wx == "dusty":
                        vis_m = (
                            float(self.rng.uniform(1500, 4500))
                            if self.rng.random() < 0.8
                            else float(self.rng.uniform(700, 990))
                        )
                    else:
                    ...
                    temp_c = round(
                        base_temp
                        + 6.0 * math.sin((h - 8) / 24.0 * 2 * math.pi)
                        + float(self.rng.normal(0, 1.5)),
                        1,
                    )
                    heat_index_c = (
                        round(temp_c + (2.0 if temp_c > 30 else 0.0), 1) if self.rng.random() > 0.02 else None
                    )
                    precip = round(float(self.rng.uniform(2.0, 18.0)), 1) if wx == "rain" else 0.0
                    wind_kmh = round(
                        float(self.rng.uniform(40.0, 52.0))
                        if wx == "windy"
                        else float(self.rng.uniform(6.0, 22.0)),
                        1,
                    )
...
def run_generator(config_path: Path | None = None, effect_scale: float = 1.0, out_dir: Path | None = None):
    ...
    print(
        f"Generating synthetic dataset (generator={cfg.generator_version}, seed={cfg.seed}, scale={effect_scale})..."
    )
...
def main():
    ...
    parser.add_argument(
        "--effect-scale", type=float, default=1.0, help="Effect scale factor (default 1.0, e.g. 0.5 or 1.5)"
    )
    parser.add_argument(
        "--all-sensitivities",
        action="store_true",
        help="Generate baseline and all sensitivity runs (0.5, 1.5)",
    )
    parser.add_argument(
        "--no-validate", action="store_true", help="Skip running validation gates after generation"
    )
